...
Bei gleichem Aufbau soll nun die Spannungsverstärkung bei variierendem $\RE$
und gleichbleibendem $\RC$ und anders herum vermessen werden. Die Messwerte
sind in Tabelle~\ref{tab:412}. Zum Vergleich mit der Näherungsformel ist eine
Spalte mit der genäherten Verstärkung eingefügt.
...
\begin{table}
	...
	\begin{tabular}{SSSSSS}
		...
		{$v = -\RC / \RE$} \\
		\hline
		%< for re, rc, dub, duc, v_mess, v_naeherung in table412: >%
		<< re >> & << rc >> & << dub >> & << duc >> & << v_mess >> & << v_naeherung
		>> \\
		%< endfor >%
	\end{tabular}
	\caption{%
