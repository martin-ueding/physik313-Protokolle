...
erfährt, also das Verhältnis von $U_\text h$ und $I_\text h$. Das Kabel wirkt
durch den Abschlusswiderstand unendlich lang, so dass $R_\text{in} = Z$ gilt.

Dies ist aus Formel \eqref{eq:1.17} zu sehen. Dort ist das Kabelende ($x = l$)
beschrieben. Wir setzen dort die rücklaufende Welle gleich null, sowie $r = 0$
ein, und erhalten:
\[
	R_\text A = \frac{U_\text h}{I_\text h}
\]
